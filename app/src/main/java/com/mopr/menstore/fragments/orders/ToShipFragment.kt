...
class ToShipFragment : Fragment() {
    ...
    private fun fetchData(userId: Int){
        lifecycleScope.launch {
            try {
                ...
                val ordersToShip : MutableList<Order> = mutableListOf()
                for (item in sortOrderByUser) if (item.status == 2) ordersToShip.add(item)
                ...
                bindOrdersToShipByUser(ordersToShip, listOrderItems, firstOrderProducts,firstOrderProductDetails,firstProductImages)
    ...
    private fun bindOrdersToShipByUser(orders: List<Order>, listOrderItems: List<List<OrderItem>>, firstOrderProducts: List<Product>, firstOrderProductDetails: List<ProductDetail>, firstProductImages: List<ProductImage>){
        if(orders.isNotEmpty() && firstOrderProducts.isNotEmpty()){
            val orderAdapter= OrderAdapter(this@ToShipFragment, orders,listOrderItems, firstOrderProducts,firstOrderProductDetails,firstProductImages, 2)
