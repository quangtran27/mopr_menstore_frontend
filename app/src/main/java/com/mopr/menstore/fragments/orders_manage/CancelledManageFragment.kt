...
import com.mopr.menstore.databinding.FragmentCancelledManageBinding
...
class CancelledManageFragment : Fragment() {
    private var param1: String? = null

    private lateinit var orderApiUtil: OrderApiUtil
    private lateinit var productApiUtil: ProductApiUtil

    private lateinit var binding: FragmentCancelledManageBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = FragmentCancelledManageBinding.inflate(layoutInflater)
        arguments?.let {
    ...
    private fun fetchData() {
        lifecycleScope.launch {
            try {
                ...
                val ordersCancelled: MutableList<Order> = mutableListOf()
                for (item in sortOrderByUser) if (item.status == 5) ordersCancelled.add(item)
    ...
    private fun bindOrdersCancelledByUser(
        orders: List<Order>,
    ...
    companion object {
        ...
        fun newInstance(param1: String) =
            CancelledManageFragment().apply {
                arguments = Bundle().apply {
                    putString(ARG_PARAM1, param1)
                }
            }

        const val TAG = "CancelledManageFragment"
    }
}