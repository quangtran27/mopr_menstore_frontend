...
import com.mopr.menstore.databinding.FragmentToShipManageBinding
...
class ToShipManageFragment : Fragment() {
    private var param1: String? = null

    private lateinit var orderApiUtil: OrderApiUtil
    private lateinit var productApiUtil: ProductApiUtil

    private lateinit var binding: FragmentToShipManageBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = FragmentToShipManageBinding.inflate(layoutInflater)
        arguments?.let {
    ...
    private fun bindOrdersToShipByUser(
        orders: List<Order>,
    ...
    private fun confirmOrder(orderId: Int,  isPaid: Int, isReviewed: Int) {
        lifecycleScope.launch {
            orderApiUtil.updateOrder(orderId, 3, isPaid, isReviewed)
            fetchData()
    ...
    companion object {
        ...
        fun newInstance(param1: String) =
            ToShipManageFragment().apply {
                arguments = Bundle().apply {
                    putString(ARG_PARAM1, param1)
                }
            }

        const val TAG = "ToShipManageFragment"
    }
}